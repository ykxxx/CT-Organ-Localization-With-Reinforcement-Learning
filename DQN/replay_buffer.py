...
class ReplayMemory(object):
    def __init__(self, max_size, state_shape, history_len, num_agents):
        ...
        self.state = np.zeros(
            (self.num_agents, self.max_size) + state_shape, dtype='float32')
        self.action = np.zeros((self.num_agents, self.max_size), dtype='int32')
        self.reward = np.zeros((self.num_agents, self.max_size), dtype='float32')
        self.isOver = np.zeros((self.num_agents, self.max_size), dtype='bool')
...
def main():
    buffer = ReplayMemory(max_size = 1000, state_shape = config.screen_dims, history_len = 4, agents = 1)